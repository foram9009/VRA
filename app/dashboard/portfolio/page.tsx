...
export default function PortfolioManager() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.coverImage) form.set('coverImage', formData.coverImage);
          ...
          <div className="bg-card w-full max-w-2xl rounded-xl border border-white/10 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <input name="slug" defaultValue={formData.slug} required placeholder="URL Slug (e.g. my-project)" className="w-full bg-background border border-white/10 rounded px-3 py-2 focus:outline-none focus:border-primary" />
              </div>

              <div className="space-y-1">
                <label className="text-xs text-text-secondary uppercase tracking-widest">Category</label>
                <select
                  name="categoryId"
                ...
                >
                  ...
                  {categories.length === 0 ? (
                    <option disabled>No categories found — create one in Categories</option>
                  ) : (
              ...
              <textarea name="description" defaultValue={formData.description} required rows={3} placeholder="Project Description (min. 10 chars)" className="w-full bg-background border border-white/10 rounded px-3 py-2 focus:outline-none focus:border-primary resize-none" />
